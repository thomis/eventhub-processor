module EventHub
  class Message
    ...
    def valid?
      ...
      REQUIRED_HEADERS.all? { |key| @header.all_keys_with_path.include?(key) && !!send(key.tr(".", "_").to_sym) }
    end
    ...
    def self.translate_status_code(code)
      case code
      when EventHub::STATUS_INITIAL then "STATUS_INITIAL"
      when EventHub::STATUS_SUCCESS then "STATUS_SUCCESS"
      when EventHub::STATUS_RETRY then "STATUS_RETRY"
      when EventHub::STATUS_RETRY_PENDING then "STATUS_RETRY_PENDING"
      when EventHub::STATUS_INVALID then "STATUS_INVALID"
      when EventHub::STATUS_DEADLETTER then "STATUS_DEADLETTER"
      when EventHub::STATUS_SCHEDULE then "STATUS_SCHEDULE"
      when EventHub::STATUS_SCHEDULE_RETRY then "STATUS_SCHEDULE_RETRY"
      when EventHub::STATUS_SCHEDULE_PENDING then "STATUS_SCHEDULE_PENDING"
      end
    end
  end
end
